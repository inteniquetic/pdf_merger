...
class PdfMergerPlugin : FlutterPlugin, MethodCallHandler {
    ...
    override fun onMethodCall(@NonNull call: MethodCall, @NonNull result: MethodChannel.Result) {
        ...
        if (call.method == "mergeMultiplePDF") {
            MergeMultiplePDF(context, result).merge(
                call.argument("paths"),
                call.argument("outputDirPath")
            )
        } else if (call.method == "createPDFFromMultipleImage") {
            CreatePDFFromMultipleImage(context, result).create(
                call.argument("paths"),
                call.argument("outputDirPath"),
                call.argument("needImageCompressor"),
